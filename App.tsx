  ...
  return (
    <div className="flex flex-col h-full bg-[#020617] border-r border-white/10">
      ...
      <nav className="flex-1 px-4 space-y-1 overflow-y-auto custom-scrollbar pt-2">
        <p className="px-3 text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4">Architecture</p>
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <item.icon size={18} className={`${isActive ? 'text-indigo-400' : 'text-slate-400 group-hover:text-white'} transition-colors duration-300 shrink-0`} />
        ...
        <div className="mt-8 pt-6 px-2 border-t border-white/10">
           <button 
            onClick={() => { onOpenUpload(); closeMobileMenu?.(); }}
            className="w-full flex items-center gap-3 px-4 py-4 rounded-2xl text-white bg-indigo-600/10 hover:bg-indigo-600 transition-all font-black border border-indigo-500/30 group relative overflow-hidden shadow-lg active:scale-95 duration-300"
           >
      ...
      {/* User Footer Section */}
      <div className="p-4 mt-auto border-t border-white/10 bg-[#020617]/50 backdrop-blur-xl">
        <div className="bg-white/[0.05] rounded-[24px] p-3 border border-white/10 flex items-center gap-3 hover:bg-white/[0.08] transition-colors cursor-default group">
          <div className="w-10 h-10 rounded-xl bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center text-indigo-400 font-black text-base shadow-inner shrink-0 group-hover:border-indigo-400/50 transition-colors">
             {user.name?.charAt(0) || 'U'}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-black text-white truncate uppercase tracking-tight leading-none mb-1.5">{user.name || 'User'}</p>
            <div className="flex items-center gap-1.5">
              <div className="relative flex items-center justify-center">
                <div className="w-1.5 h-1.5 rounded-full bg-green-500"></div>
                <div className="absolute w-1.5 h-1.5 rounded-full bg-green-500 animate-ping opacity-60"></div>
              </div>
              <p className="text-[8px] text-slate-400 uppercase font-black tracking-widest leading-none truncate">Architect Lvl {Math.floor((user.readinessScore || 0)/10)}</p>
            </div>
          </div>
          <button 
            onClick={onLogout}
      ...
      <div className="flex items-center gap-4 md:gap-6">
        ...
        <div className="flex flex-col justify-center">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] leading-none">Neural Connection Active</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex min-h-screen bg-[#020617] text-slate-200">
        ...
        {user?.isOnboardingComplete && isMobileMenuOpen && (
          <div className="fixed inset-0 z-[100] lg:hidden">
            ...
            ></div>
            <aside className="absolute top-0 left-0 bottom-0 w-[280px] bg-[#020617] animate-in slide-in-from-left duration-500 flex flex-col shadow-2xl border-r border-white/10">
              <div className="flex justify-end p-5">
                 <button 
                  onClick={() => setIsMobileMenuOpen(false)} 
                  className="p-2.5 text-slate-400 hover:text-white rounded-xl hover:bg-white/10 transition-all active:scale-95"
