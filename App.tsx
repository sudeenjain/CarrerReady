...
import { apiFetch, clearTokens, getAccessToken } from './src/api/client';
...
const SidebarContent = ({ user, onOpenUpload, onLogout, closeMobileMenu }: { user: UserProfile, onOpenUpload: () => void, onLogout: () => void, closeMobileMenu?: () => void }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[var(--bg-main)] border-r border-[var(--border-color)] transition-colors duration-300">
      {/* Brand Section */}
      <div className="px-6 py-8 flex items-center">
        <Link to="/" onClick={closeMobileMenu} className="flex items-center gap-3.5 group cursor-pointer">
          ...
          <div className="flex flex-col">
            <h1 className="font-display font-black text-[var(--text-main)] tracking-tighter leading-none text-base transition-colors">CareerReadyAI</h1>
        ...
        {menuItems.map((item) => {
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-4 py-3 rounded-2xl transition-all duration-300 group relative overflow-hidden ${isActive
                  ? 'bg-[var(--glass-bg)] text-[var(--text-main)] border border-[var(--border-color)] shadow-sm'
                  : 'text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--glass-bg)] border border-transparent'
                }`}
            >
              {isActive && (
      ...
      {/* User Footer Section */}
      <div className="p-4 mt-auto border-t border-[var(--border-color)] bg-[var(--nav-bg)] backdrop-blur-xl transition-colors">
        <div className="bg-[var(--glass-bg)] rounded-[24px] p-3 border border-[var(--border-color)] flex items-center gap-3 transition-colors cursor-default group">
  ...
  return (
    <header className="h-20 nav-blur sticky top-0 z-30 px-6 md:px-10 flex items-center justify-between">
      <div className="flex items-center gap-4 md:gap-6">
        <button
          onClick={onOpenMenu}
          className="lg:hidden p-2.5 text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--glass-bg)] rounded-xl border border-[var(--border-color)] transition-all active:scale-95 flex items-center justify-center"
          aria-label="Open Menu"
        >
          <Menu size={18} />
        </button>
        <div className="flex flex-col justify-center">
          <h2 className="text-xl md:text-2xl font-display font-black text-[var(--text-main)] tracking-tighter leading-none mb-1.5 transition-colors">{pageTitle}</h2>
      ...
      <div className="flex items-center gap-3 md:gap-4">
        <Link
          to="/jobs"
          className="hidden sm:flex items-center gap-2.5 px-6 py-2.5 bg-[var(--text-main)] text-[var(--bg-main)] rounded-xl transition-all shadow-xl font-black text-[9px] uppercase tracking-widest active:scale-95 border border-[var(--border-color)]"
        >
          <Briefcase size={14} />
          <span>Match Feed</span>
        </Link>
        <div className="hidden sm:block w-px h-8 bg-[var(--border-color)] mx-1"></div>
        <ThemeToggle />
        <button
          className="p-2.5 text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--glass-bg)] rounded-xl transition-all group flex items-center justify-center"
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const boot = async () => {
      ...
      if (!token) return;
      try {
        const me = await apiFetch<{ email: string; full_name?: string | null }>('/api/v1/users/me', { method: 'GET' });
        const isOnboardingComplete = localStorage.getItem('cr_onboarding_complete') === 'true';
        setUser({
          ...INITIAL_USER,
  ...
  const handleAuthenticated = (loginData: { name: string; email: string }) => {
    const isOnboardingComplete = localStorage.getItem('cr_onboarding_complete') === 'true';
    setUser({
      ...INITIAL_USER,
      name: loginData.name,
    ...
    // Show welcome toast after a brief delay so navigation completes first
    setTimeout(() => {
      showWelcomeToast(loginData.name || loginData.email.split('@')[0]);
    }, 800);
  };

  const handleOnboardingComplete = (data: Partial<UserProfile>) => {
    if (!user) return;
    updateUser({ ...data, isOnboardingComplete: true });
    localStorage.setItem('cr_onboarding_complete', 'true');
    void apiFetch('/api/v1/users/me', {
      method: 'PATCH',
      body: JSON.stringify({ full_name: user.name || data.name, target_role: data.targetRole }),
    }).catch(() => { });
  };
  ...
  const handleSyncResult = (result: { skills: Skill[], projects?: Project[] }) => {
    ...
    user.currentSkills.forEach(s => skillMap.set(s.name.toLowerCase(), s));
    result.skills.forEach(s => skillMap.set(s.name.toLowerCase(), s));

    const projectMap = new Map();
    user.projects.forEach(p => projectMap.set(p.name.toLowerCase(), p));
    if (result.projects) {
      ...
      <Router>
        <div className="flex min-h-screen bg-[var(--bg-main)] text-[var(--text-main)] transition-colors duration-300">
          {user?.isOnboardingComplete && (
            <aside className="w-72 border-r border-[var(--border-color)] h-screen sticky top-0 flex flex-col hidden lg:flex shrink-0 z-40">
          ...
          {user?.isOnboardingComplete && isMobileMenuOpen && (
            <div className="fixed inset-0 z-[100] lg:hidden">
              <div
                className="absolute inset-0 bg-[var(--bg-main)]/90 backdrop-blur-md animate-in fade-in duration-300"
                onClick={() => setIsMobileMenuOpen(false)}
              ></div>
              <aside className="absolute top-0 left-0 bottom-0 w-[280px] bg-[var(--bg-main)] animate-in slide-in-from-left duration-500 flex flex-col shadow-2xl border-r border-[var(--border-color)] transition-colors duration-300">
                <div className="flex justify-end p-5">
                  <button
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="p-2.5 text-[var(--text-muted)] hover:text-[var(--text-main)] rounded-xl hover:bg-[var(--glass-bg)] transition-all active:scale-95"
          ...
          <main className="flex-1 flex flex-col min-w-0">
            ...
            <div className={`flex-1 overflow-x-hidden ${user?.isOnboardingComplete ? 'p-6 md:p-10 max-w-[1440px] mx-auto w-full main-content-area' : ''}`}>
              <Routes>
                <Route path="/landing" element={user ? <Navigate to={user.isOnboardingComplete ? "/" : "/onboarding"} /> : <Landing />} />
                <Route path="/auth" element={user ? <Navigate to={user.isOnboardingComplete ? "/" : "/onboarding"} /> : <Auth onAuthenticated={handleAuthenticated} />} />
        ...
        {isUploadOpen && <ResumeScanner onSkillsExtracted={(result) => handleSyncResult(result)} onClose={() => setIsUploadOpen(false)} />}
