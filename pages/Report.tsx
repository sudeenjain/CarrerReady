  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000 pb-20">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-black text-[var(--text-main)] tracking-tight">Readiness Audit</h2>
          <p className="text-[var(--text-muted)] font-medium italic">Verified snapshot for {user.name}</p>
        </div>
        <div className="flex gap-3 relative">
          ...
          <button 
            onClick={handleShare}
            className="p-3 bg-[var(--bg-card)] border border-slate-200 dark:border-slate-800 rounded-2xl text-slate-400 hover:text-slate-800 dark:hover:text-[var(--text-main)] dark:text-white transition-colors shadow-sm"
          >
             <Share2 size={20} />
          </button>
          <button 
            onClick={() => window.print()}
            className="flex items-center gap-2 px-6 py-3 bg-[var(--bg-card)] border border-slate-200 dark:border-slate-800 text-[var(--text-main)] rounded-2xl font-bold shadow-lg hover:bg-slate-50 transition-all"
          >
             <FileText size={20} />
             Print Report
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          {/* Main Mastery Gauge */}
          <div className="bg-[var(--bg-card)] rounded-[32px] p-10 border border-[var(--border-color)] shadow-xl relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:scale-110 transition-transform">
               <Award size={200} className="dark:text-[var(--text-main)] dark:text-white" />
            </div>
            <div className="flex flex-col md:flex-row items-center gap-12 relative z-10">
               ...
               <div className="flex-1 space-y-6">
                  <h3 className="text-4xl font-black text-[var(--text-main)] tracking-tight">Hiring Profile</h3>
                  <div className="grid grid-cols-3 gap-4">
                     <div className="text-center"><p className="text-[10px] font-black uppercase text-slate-400">Match</p><p className="text-xl font-black text-[var(--text-main)]">{breakdown.skillPoints}</p></div>
          ...
          <div className="bg-[var(--glass-bg)] rounded-[32px] p-8 text-[var(--text-main)] shadow-2xl relative overflow-hidden border border-[var(--border-color)]">
             ...
             <div className="relative z-10 space-y-6">
                ...
                {latexCode ? (
                  <div className="space-y-6 animate-in zoom-in-95">
                     <div className="p-4 bg-black/40 rounded-2xl border border-[var(--border-color)] font-mono text-[10px] overflow-x-auto custom-scrollbar h-48 whitespace-pre">
                        {latexCode}
                     </div>
                     ...
                     <div className="p-5 bg-indigo-500/10 rounded-2xl border border-indigo-500/20">
                        ...
                        <ol className="text-xs text-indigo-200 space-y-2 list-decimal list-inside font-medium">
                          ...
                          <li>Open <a href="https://www.overleaf.com" target="_blank" className="text-[var(--text-main)] dark:text-white underline">Overleaf</a> and create a "New Project" → "Blank Project".</li>
          ...
          <div className="bg-[var(--bg-card)] rounded-[32px] p-8 border border-[var(--border-color)] shadow-sm space-y-8">
             <div className="flex items-center gap-3">
                <ShieldCheck className="text-indigo-600" />
                <h3 className="text-xl font-black text-[var(--text-main)]">Audit Feedback & Recommendations</h3>
             </div>
        ...
        <div className="space-y-8">
           <div className="bg-[var(--glass-bg)] rounded-[32px] p-8 text-[var(--text-main)] shadow-2xl">
           ...
           <div className="bg-[var(--bg-card)] rounded-[32px] p-8 border border-[var(--border-color)] shadow-lg space-y-6">
