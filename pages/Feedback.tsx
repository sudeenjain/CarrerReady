  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-700 pb-20">
      <div className="text-center space-y-4">
        ...
        <h2 className="text-4xl md:text-5xl font-black text-[var(--text-main)] tracking-tighter leading-none">
          Evolve the <span className="text-indigo-600">Engine.</span>
        </h2>
        <p className="text-[var(--text-muted)] font-medium max-w-lg mx-auto leading-relaxed">
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-8 items-start">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-[var(--glass-bg)] rounded-[32px] p-8 text-[var(--text-main)] shadow-2xl relative overflow-hidden group border border-[var(--border-color)]">
            ...
            <div className="bg-amber-500/5 border border-amber-500/20 rounded-[32px] p-6 space-y-4 animate-in slide-in-from-left-4 duration-500">
               ...
               <div className="space-y-3 max-h-60 overflow-y-auto custom-scrollbar pr-2">
                  {pendingItems.map((item) => (
                    <div key={item.id} className="p-4 rounded-2xl bg-[var(--bg-card)] border border-amber-500/10 shadow-sm space-y-3">
          ...
          <div className="bg-[var(--bg-card)] rounded-[28px] p-6 border border-[var(--border-color)] shadow-sm">
        ...
        <div className="md:col-span-3">
          {(status === 'success' || status === 'locally_saved') ? (
            <div className="bg-[var(--bg-card)] rounded-[40px] p-12 border border-[var(--border-color)] shadow-xl text-center space-y-6 animate-in zoom-in-95 duration-500">
              ...
              <div>
                ...
                <p className="text-[var(--text-muted)] font-medium mt-2 leading-relaxed">
                  {status === 'success' 
              ...
              <button 
                ...
                className="px-10 py-4 bg-[var(--input-bg)] text-slate-600 dark:text-slate-300 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] hover:bg-slate-200 dark:hover:bg-slate-700 transition-all border border-transparent dark:border-slate-700"
              >
                Draft Another Packet
              </button>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="bg-[var(--bg-card)] rounded-[40px] p-8 md:p-12 border border-[var(--border-color)] shadow-xl space-y-8">
              <div className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <MessageSquare size={16} className="absolute left-4 top-5 text-[var(--text-muted)]" />
                    <textarea 
                      name="message"
                      required
                      rows={5}
