...
const Roadmap: React.FC<{ user: UserProfile, onUpdateUser: (u: UserProfile) => void }> = ({ user, onUpdateUser }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6">
        ...
        <div className="text-center">
          ...
          <p className="text-slate-400 text-sm font-bold mt-2">Constructing your 45-day daily mastery sequence...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-24 animate-in fade-in duration-1000">
      {isOffline && (
        <div className="p-4 bg-amber-500/20 border border-amber-500/30 rounded-2xl flex items-center gap-3 text-amber-400 text-xs font-bold">
          <WifiOff size={16} />
      ...
      <div className="bg-[#0f172a]/80 backdrop-blur-2xl rounded-[48px] p-10 border border-white/10 shadow-2xl flex flex-col lg:flex-row items-center justify-between gap-12 relative overflow-hidden group">
        ...
        <div className="relative z-10 space-y-6 text-center lg:text-left flex-1">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-indigo-500/20 text-indigo-400 rounded-xl text-[10px] font-black uppercase tracking-[0.25em] border border-indigo-500/30">
        ...
        <div className="relative w-52 h-52 shrink-0">
           ...
           <div className="absolute inset-0 flex flex-col items-center justify-center">
             ...
             <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Total Completion</span>
           </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-4">
        {phases.map((p, i) => (
          ...
          >
            <span className="text-[9px] font-black opacity-60 uppercase tracking-[0.3em]">Module 0{i+1}</span>
      ...
      <div className="space-y-6">
        {filteredSteps.length === 0 && (
           <div className="py-24 text-center bg-[#0f172a]/40 rounded-[48px] border border-white/10">
              <Loader2 className="animate-spin mx-auto text-slate-400 mb-6" size={40} />
              <p className="text-[12px] font-black uppercase tracking-[0.4em] text-slate-400">Syncing Phase Data...</p>
           </div>
        )}
          ...
          return (
            <div key={step.day} className="group/day">
              <div className={`rounded-[40px] border transition-all duration-500 overflow-hidden ${
                isExpanded ? 'bg-[#0f172a] border-indigo-500/50 shadow-2xl' : 'bg-white/5 border-white/10 hover:bg-white/10'
              }`}>
                {/* Protocol Header */}
                <div 
                ...
                >
                  <div className="flex items-center gap-8 flex-1 min-w-0">
                    <div className={`w-16 h-16 rounded-[24px] flex flex-col items-center justify-center shrink-0 border transition-all duration-500 ${
                      isDone 
                        ...
                        : isExpanded ? 'bg-indigo-600 border-indigo-400 text-white' : 'bg-white/5 border-white/10 text-slate-400'
                    }`}>
                  ...
                  <div className="flex items-center gap-6">
                    <div className="hidden sm:flex flex-col items-end">
                       <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Loadout Progress</p>
                    ...
                    <ChevronDown className={`transition-transform duration-500 text-slate-400 ${isExpanded ? 'rotate-180 text-white' : ''}`} size={24} />
                  </div>
                </div>
                ...
                {isExpanded && (
                  <div className="px-8 pb-10 pt-2 animate-in slide-in-from-top-4 duration-500 space-y-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
                      ...
                      >
                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${
                           isTaskComplete(step.day, 'learn') ? 'bg-blue-500 text-white' : 'bg-blue-500/20 text-blue-400 border border-blue-500/30'
                         }`}>
                            {isTaskComplete(step.day, 'learn') ? <CheckCircle size={24} /> : <Brain size={24} />}
                         </div>
                         <h5 className="text-[10px] font-black uppercase tracking-widest text-slate-400">01. Theoretical Scan (Learn)</h5>
                         <p className={`text-xs font-bold leading-relaxed transition-colors ${isTaskComplete(step.day, 'learn') ? 'text-blue-100' : 'text-slate-200'}`}>
                           {step.learningTask}
                         </p>
                      </div>
                      ...
                      >
                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${
                           isTaskComplete(step.day, 'practice') ? 'bg-indigo-500 text-white' : 'bg-indigo-500/20 text-indigo-400 border border-indigo-500/30'
                         }`}>
                            {isTaskComplete(step.day, 'practice') ? <CheckCircle size={24} /> : <Zap size={24} />}
                         </div>
                         <h5 className="text-[10px] font-black uppercase tracking-widest text-slate-400">02. Combat Drills (Practice)</h5>
                         <p className={`text-xs font-bold leading-relaxed transition-colors ${isTaskComplete(step.day, 'practice') ? 'text-indigo-100' : 'text-slate-200'}`}>
                           {step.practiceTask}
                         </p>
                      </div>
                      ...
                      >
                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${
                           isTaskComplete(step.day, 'build') ? 'bg-purple-500 text-white' : 'bg-purple-500/20 text-purple-400 border border-purple-500/30'
                         }`}>
                            {isTaskComplete(step.day, 'build') ? <CheckCircle size={24} /> : <Hammer size={24} />}
                         </div>
                         <h5 className="text-[10px] font-black uppercase tracking-widest text-slate-400">03. Systems Construct (Build)</h5>
                         <p className={`text-xs font-bold leading-relaxed transition-colors ${isTaskComplete(step.day, 'build') ? 'text-purple-100' : 'text-slate-200'}`}>
                           {step.buildingTask}
                         </p>
                      </div>
                      ...
                      >
                         <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${
                           isTaskComplete(step.day, 'review') ? 'bg-green-500 text-white' : 'bg-green-500/20 text-green-400 border border-green-500/30'
                         }`}>
                            {isTaskComplete(step.day, 'review') ? <CheckCircle size={24} /> : <Eye size={24} />}
                         </div>
                         <h5 className="text-[10px] font-black uppercase tracking-widest text-slate-400">04. Quality Audit (Review)</h5>
                         <p className={`text-xs font-bold leading-relaxed transition-colors ${isTaskComplete(step.day, 'review') ? 'text-green-100' : 'text-slate-200'}`}>
                           {step.reviewTask}
                         </p>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row items-stretch gap-6">
                       <div className="flex-1 p-8 bg-indigo-500/10 rounded-[40px] border border-indigo-500/20 flex items-center gap-8">
                          <div className={`w-16 h-16 rounded-[24px] flex items-center justify-center shrink-0 shadow-inner ${isDone ? 'bg-green-500 text-white' : 'bg-white/10 text-slate-400'}`}>
                    ...
                    {step.day % 7 === 0 && (
                      <div className="p-8 bg-amber-500/10 border border-amber-500/20 rounded-[40px] flex items-center gap-6 group/check relative overflow-hidden">
                         <div className="absolute top-0 right-0 p-8 opacity-[0.05] group-hover/check:scale-110 transition-transform"><Award size={100} /></div>
      ...
      <div className="bg-slate-900 rounded-[48px] p-10 border border-white/10 shadow-2xl relative overflow-hidden group">
         ...
         <div className="relative z-10 flex flex-col lg:flex-row items-center gap-12">
            ...
            <div className="hidden lg:flex items-center gap-4 bg-black/20 px-8 py-4 rounded-3xl border border-white/10">
               <div className="text-center">
                  <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Estimated Load</p>
