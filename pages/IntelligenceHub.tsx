...
const IntelligenceHub: React.FC = () => {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-12 pb-24 animate-in fade-in duration-1000">
      {/* Header */}
      <div className="bg-[#0f172a]/60 backdrop-blur-xl rounded-[40px] p-10 border border-[var(--border-color)] shadow-2xl relative overflow-hidden group">
          ...
          <button
            ...
            className={`p-6 rounded-[32px] border transition-all flex flex-col items-center gap-4 group relative ${
              activeCategory === cat.id 
                ...
                : 'bg-white/5 text-slate-400 border-[var(--border-color)] hover:border-[var(--border-color)]'
            }`}
          >
      ...
      {activeCategory === 'signals' ? (
        <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
          ...
          {localSignals.length === 0 ? (
            <div className="bg-[#0f172a]/40 border border-[var(--border-color)] rounded-[40px] p-20 text-center space-y-4">
              ...
              {localSignals.map((signal) => (
                <div key={signal.id} className="p-8 rounded-[40px] bg-[#0f172a]/60 border border-indigo-500/20 backdrop-blur-xl group hover:border-indigo-400 transition-all flex flex-col justify-between">
                  <div className="space-y-6">
                    ...
                    <div className="p-5 rounded-2xl bg-black/20 border border-[var(--border-color)] text-slate-300 text-sm italic font-medium leading-relaxed">
                      "{signal.message}"
                    </div>
                  </div>
                  ...
                  <div className="mt-8 pt-6 border-t border-[var(--border-color)] flex items-center justify-between">
                    ...
                    <button 
                      ...
                      className="p-2 text-slate-500 hover:text-red-400 transition-colors bg-white/5 rounded-xl border border-[var(--border-color)]"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {suggestionCategories.find(c => c.id === activeCategory)?.items.map((item, idx) => (
            <div 
              key={idx} 
              className="p-8 rounded-[40px] bg-[#0f172a]/40 border border-[var(--border-color)] backdrop-blur-sm group hover:border-indigo-500/30 transition-all relative overflow-hidden flex flex-col justify-between"
            >
               ...
               <div className="mt-8 pt-6 border-t border-[var(--border-color)] flex items-center justify-between opacity-50 group-hover:opacity-100 transition-opacity">
