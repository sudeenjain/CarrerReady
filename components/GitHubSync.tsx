  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) throw new Error('GitHub API rate limit exceeded. Check your VITE_GITHUB_TOKEN.');
      ...
      setStatus(`Found ${repos.length} repositories. Analyzing profile and skills with AI...`);

      // 3. Analyze with analysisService (Gemini under the hood)
      const extractedResult = await analysisService.analyzeGitHubRepos(repos, username, userProfile);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/20 dark:bg-black/50 backdrop-blur-sm animate-in fade-in duration-300">
      <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
        <div className="p-6 border-b border-slate-100 flex items-center justify-between bg-[var(--bg-card)] text-[var(--text-main)]">
            ...
            <button
              ...
              className={`flex-1 py-4 rounded-2xl font-bold flex items-center justify-center gap-3 transition-all ${
                isSyncing || !username.trim()
                  ...
                  : 'bg-[var(--bg-card)] text-[var(--text-main)] shadow-lg shadow-slate-200 hover:scale-[1.02] active:scale-95'
              }`}
            >
              {isSyncing ? (
                <>
