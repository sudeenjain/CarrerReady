...
type ResumeScannerProps = {
  onSkillsExtracted: (result: { skills: Skill[], projects?: any[] }) => void;
  onClose: () => void;
};
...
export function ResumeScanner({ onSkillsExtracted, onClose }: ResumeScannerProps) {
  ...
  const validateResumeContent = (content: string) => {
    ...
    if (words < 20) return "⚠️ Invalid document detected. Please paste at least a brief summary of your skills and experience.";
    ...
    if (found.length < 1) return "⚠️ Invalid document detected. This looks like a generic text. Please upload your RESUME or profile summary.";
    return null;
  };
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const result = await analysisService.extractSkillsFromResume(text) as any;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 dark:bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-[var(--bg-card)] w-full max-w-2xl rounded-[32px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 border border-white/20">
        <div className="p-6 border-b border-[var(--border-color)] flex items-center justify-between gradient-bg text-white">
          ...
          {step === 'fallback' && (
            <div className="space-y-6 animate-in slide-in-from-bottom-4">
              ...
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 flex items-center gap-4">
                       <Github className="text-[var(--text-main)]" size={24} />
              ...
              <div className="flex gap-4">
                <button onClick={() => setStep('upload')} className="flex-1 py-4 bg-[var(--input-bg)] text-[var(--text-muted)] rounded-2xl font-black text-xs uppercase tracking-widest border border-transparent dark:border-slate-700">Try Resume Again</button>
            ...
            <div className="space-y-6">
              ...
              {benchmark && (
                <div className="p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 rounded-2xl">
                  <h4 className="text-xs font-black text-indigo-800 dark:text-indigo-300 uppercase tracking-widest mb-2 flex items-center gap-2">
                    <Sparkles size={14} /> Advanced Real-World Benchmark
                  </h4>
              ...
              <button 
                onClick={() => { onSkillsExtracted({ skills: extractedSkills, projects: extractedProjects }); onClose(); }}
